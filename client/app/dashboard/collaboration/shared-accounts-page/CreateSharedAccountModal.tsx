...
export default function CreateSharedAccountModal({ open, onClose, onCreate }: CreateSharedAccountModalProps) {
              ...
              <CardContent className="space-y-5 px-6 py-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h2 className="text-lg font-bold text-foreground">Create Shared Account</h2>
                      <p className="text-xs text-foreground/60">Start a live shared balance workspace</p>
                    </div>
                  </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <input required value={name} onChange={(e) => setName(e.target.value)} placeholder="Account name" className="w-full rounded-xl border border-border/80 bg-background/60 px-4 py-2.5 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20" />
