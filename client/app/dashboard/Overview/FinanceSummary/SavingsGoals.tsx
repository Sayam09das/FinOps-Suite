  ...
  return (
    <Card className="h-[420px] rounded-3xl border-2 border-border/50 shadow-xl hover:shadow-2xl">
      ...
      <CardContent className="space-y-6">
          ...
          {goals.length === 0 ? (
            ...
            <>
              {goals.map((goal, index) => (
                <div key={goal.name || index} className="group p-4 rounded-2xl hover:bg-muted/50 transition-all border hover:border-primary/30">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                      ...
                      <span>ETA: {calculateETA(goal.progress, goal.target)}</span>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div className="p-5 bg-green-500/10 rounded-2xl border border-green-500/20 shadow-lg">
          <div className="flex items-center justify-between">
            ...
            <div className="p-3 bg-green-500/20 rounded-2xl shadow-md">
              <TrendingUp className="h-6 w-6 text-green-500" />
            </div>
          </div>
          <p className="text-xs text-muted-foreground mt-2">
            {savingsRate >= 20 ? "Excellent! Above 20% target" : "Target: 20% savings rate"}
          </p>
        </div>
      </CardContent>
    </Card>
